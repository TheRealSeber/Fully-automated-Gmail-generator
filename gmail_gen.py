...
import random, os , time, subprocess, csv , requests, string, json, traceback, multiprocessing, pandas as pd, undetected_chromedriver as uc
...
def main():
    ...
    class SmsVerification:
        ...
        def __init__(self) -> None:
            ...
            self.sms_countries = [15,16,48]
            self.sms_current_activation_id = ''
            self.sms_activation_status = ''
            self.sms_number_data = ''
            self.sms_number = ''
            self.sms_is_new_number = False
            self.can_be_reused = True
            self.number_of_use = 0

        def getNumberByMyself(self):
            # operators = ['plus', 'play', 'tmobile', 'orange'] # trying to get PL phone
            # random.shuffle(operators)
            # for operator in operators:
            #     for x in range(2):
            #         try:
            #             number_data = self.sa.getNumberV2(service='go', country=15, operator=operator)
            #             number_data['phoneNumber']
            #             return number_data
            #         except (KeyError, TypeError):
            #             pass3
            
            countries = [25] # 25 Laos, 85 Moldavia, 141 Slovakia, 63 Czechia
            country = random.choice(countries)
            if country == 85: # Moldavia
                operators = ['orange']
            elif country == 141: # Slovakia
                operators = ['any']
            elif country == 63: # Czechia
                operators = ['o2', 'tmobile']
            elif country == 25:
                operators = ['any']
            if country == 25:
                self.can_be_reused = False
            else:
                self.can_be_reused = True
            for x in range(3):
                for operator in operators:
                    try:
                        number_data = self.sa.getNumberV2(service='go', country=country, operator=operator)
                        number_data['phoneNumber']
                        return number_data
                    except (KeyError, TypeError):
                        pass
    ...
    class Gmail_gen_forward:
        ...
        def __init__(self, gender, master_mail = '') -> None:
            ...
            self.sms_activate_api = Tasks.settings['sms_activate_api_key']
        ...
        def _dummy_send(self, element, word):    
            for character in word:
                element.send_keys(character)
                time.sleep(random.uniform(0.04,0.275))
        ...
        def first_step_register(self):
            ...
            if self._is_username_valid():
                print(f'[{The_time()}] - Data validated.')
        ...
        def sms_verification(self, smsapi):
            try:
                if smsapi.sms_current_activation_id == '':
                    ...
                    smsapi.number_of_use = 0
                elif 'STATUS_OK' in smsapi.sms_activation_status and smsapi.can_be_reused:
                    smsapi.sms_is_new_number = False
                    if smsapi.number_of_use == 2:
                ...
                elif 'STATUS_CANCEL' in smsapi.sms_activation_status or not smsapi.can_be_reused:
                    smsapi.sms_number_data = smsapi.getNumberByMyself()
                    smsapi.sms_current_activation_id = smsapi.sms_number_data['activationId']
                    smsapi.sms_number = f'+{smsapi.sms_number_data["phoneNumber"]}'
                    smsapi.sms_is_new_number = True
                        ...
                        if time.time() > timeout:
                            ...
                            if 'STATUS_OK' in smsapi.sms_activation_status:
                                ...
                                if EC.url_contains('webpersonaldetails')(self.driver):
                                    is_successfull = True
                                    smsapi.number_of_use += 1
                                    self.phone = smsapi.sms_number
                                break
    ...
    class Tasks:
        ...
        def master_webdriver(self, email=settings['master_mail']):
            ...
            try:
                ...
                option.add_argument('--user-data-dir={}\\masters profiles\\{}'.format(os.path.dirname(os.path.abspath(__file__)), email))
                ...
                print(f'[{The_time()}] - Adding filter to master driver.\n')
        ...
        def logs(self):
            if self.what_we_do == 'gen_and_forward':
                headers = ['task_status', 'first_name', 'last_name', 'gender', 'email', 'password', 'recovery_email', 'forwarded_to', 'phone']
                log_name = f'logs/gen-and-forward-logs-{self.today}.csv'
            elif self.what_we_do == 'gen':
                headers = ['task_status', 'first_name', 'last_name', 'gender', 'email', 'password', 'recovery_email', 'phone']
        ...
        def single_task_log(self, task):
            if 'orward' in task.task_status:
                headers = ['first_name', 'last_name', 'gender', 'email', 'password', 'recovery_email']
                if not os.path.isfile(f'{os.path.dirname(os.path.abspath(__file__))}\\mails forwarded to by masters\\{task.forwarded_to}.csv'):
                    with open(f'{os.path.dirname(os.path.abspath(__file__))}\\mails forwarded to by masters\\{task.forwarded_to}.csv', 'w', newline='', encoding="utf-8") as csvfile:
                        writer = csv.DictWriter(csvfile, fieldnames=headers, extrasaction='ignore')
                        writer.writeheader()
                        to_save_data = task.__dict__
                        if self.what_we_do == 'gen_and_forward':
                    ...
                    with open(f'{os.path.dirname(os.path.abspath(__file__))}\\mails forwarded to by masters\\{task.forwarded_to}.csv', 'a', newline='', encoding="utf-8") as csvfile:
                        writer = csv.DictWriter(csvfile, fieldnames=headers, extrasaction='ignore')
                        to_save_data = task.__dict__
                        if self.what_we_do == 'gen_and_forward':
                            to_save_data['email'] = to_save_data['username'] + '@gmail.com'
    ...
    def get_masters():
            print('\n0. Select all')
            folder = f'{os.path.dirname(os.path.abspath(__file__))}\\masters profiles'
            all_masters = {}
            was_mastercsv = False
            ...
            try:
                ...
                if tasks.current_master_mail != task.forwarded_to:
                    ...
                    if tasks.current_master_mail == '':
                        ...
                        tasks.master_webdriver(email=task.forwarded_to)
                changing_ip()
                p = multiprocessing.Process(target=task.start_webdriver())
                p.start()
                p.join(120)
                if p.is_alive():
                    p.kill()
                    raise TimeoutException("Driver was opening too long")
                # task.start_webdriver()
                task.first_step_register()
                task.sms_verification(smsapi)
    ...
    def create_master_gmail_profiles():
        ...
        masters_that_exist = os.listdir(f'{os.path.dirname(os.path.abspath(__file__))}\\masters profiles')

        for x in range(len(masters_to_do)):
            if masters_to_do[x]['email'] not in masters_that_exist:
                changing_ip()
                try:
                    print(f"\n{x+1}. Making {masters_to_do[x]['email']} profile.\n")
                    option = uc.ChromeOptions()
                    option.add_argument(f"--window-size={random.randint(1250,1300)},{random.randint(1060,1130)}")
                    option.add_argument('--user-data-dir={}\\masters profiles\\{}'.format(os.path.dirname(os.path.abspath(__file__)),masters_to_do[x]['email']))
